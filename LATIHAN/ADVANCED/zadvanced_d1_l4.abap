REPORT  Z_ADVANCED_D1_L4   NO STANDARD PAGE HEADING .
...
*---------------------------------------------------------------------*
FORM fm_collect_data.
SELECT a~ebeln a~aedat a~bsart a~bukrs a~lifnr b~ebelp b~matnr b~txz01 b~menge b~meins
  into CORRESPONDING FIELDS OF TABLE gi_header from ekko as a join ekpo as b on a~ebeln = b~ebeln
  where b~matnr in so_matnr order by a~ebeln DESCENDING.
  "Select PO Header Dahulu
*  SELECT ebeln aedat bsart bukrs lifnr
...
"Select PO Header Dahulu
*IF lines( gi_ekko ) > 0.
*SELECT ebeln ebelp matnr txz01 menge meins
